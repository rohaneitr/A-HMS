 ...
 */
// -------------------------------------------------------------------------
// CRYPTOGRAPHIC HARDENING — PASS-001 Remediation
// -------------------------------------------------------------------------
// Hash method: bcrypt remains appropriate for CI3 + Ion Auth 2.5.2.
// Argon2 requires PHP 7.2+ and ion_auth_model changes not in scope here.
$config['hash_method']          = 'bcrypt';

// SECURITY CHANGE: bcrypt_default_cost raised from 10 → 12.
// Bcrypt cost is exponential: each increment doubles the work factor.
// At cost 10 (PHP default), a modern GPU can test ~20,000 hashes/sec.
// At cost 12, that drops to ~5,000 hashes/sec, making a full dictionary
// attack on a stolen hash database ~4x harder.
// Benchmark your server before deployment:
//   https://gist.github.com/Indigo744/24062e07477e937a279bc97b378c3402
$config['bcrypt_default_cost']  = 12;

// SECURITY CHANGE: bcrypt_admin_cost raised from 12 → 14.
// Super Admin and Admin accounts are the highest-value targets. At cost 14,
// the work factor is 16x harder than cost 10 (the original default).
// This specifically addresses the Super Admin's existing $2y$08$ hash,
// which was generated with cost 8 — dangerously low by modern standards.
// New Super Admin passwords will be hashed with cost 14 automatically.
$config['bcrypt_admin_cost']    = 14;

// SECURITY CHANGE: default_rounds raised from 8 → 12 to align with
// bcrypt_default_cost. Although Ion Auth 2.5.2 uses bcrypt_default_cost
// as the authoritative value when hash_method = 'bcrypt', this field is
// read by legacy code paths in some forks. Aligning it prevents regression
// if the hash_method is ever temporarily switched.
$config['default_rounds']       = 12;

// random_rounds remains FALSE. Randomising the cost factor makes it
// impossible to predict whether a given hash used cost 12 or 14, but it
// also makes the performance of the login endpoint unpredictable and
// complicates monitoring for timing-based anomaly detection.
$config['random_rounds']        = FALSE;

// SECURITY CHANGE: min_rounds raised from 5 → 12. With random_rounds=FALSE
// this value is not directly used, but it acts as a safety floor. If
// random_rounds is ever enabled, it prevents the system from ever generating
// a hash weaker than cost 12.
$config['min_rounds']           = 12;

// max_rounds set to 14 to match bcrypt_admin_cost ceiling.
$config['max_rounds']           = 14;

$config['salt_prefix']          = version_compare(PHP_VERSION, '5.3.7', '<') ? '$2a$' : '$2y$';

// -------------------------------------------------------------------------
// PASSWORD POLICY — PASS-001 Remediation
// -------------------------------------------------------------------------
// SECURITY CHANGE: min_password_length raised from 5 → 10.
// The current Super Admin password is "12345" — exactly 5 characters,
// meeting the previous minimum. A minimum of 10 characters forces users
// to choose passwords that are statistically harder to brute-force via
// dictionary attacks, even before hashing is considered.
// Note: This minimum is enforced by the form_validation rules in Auth.php
// and in each module's registration controller. It is NOT enforced by
// Ion Auth itself — you must ensure all registration forms apply this rule.
$config['min_password_length']  = 10;

// SECURITY CHANGE: max_password_length raised from 20 → 72.
// Bcrypt internally truncates input at 72 bytes. There is no security
// benefit in capping below that, and doing so prevents users from choosing
// secure passphrases. We set 72 as the explicit maximum to be transparent
// about bcrypt's underlying behaviour without allowing unbounded input
// (which could be used as a DoS vector by submitting megabyte-length strings).
$config['max_password_length']  = 72;
