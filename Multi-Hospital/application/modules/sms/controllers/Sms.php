...
class Sms extends MX_Controller
{
    ...
    function sendSms($to, $message, $data = array())
    {
        $this->load->library('ssl_gateway');
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                foreach ($value as $phone => $msg) {
                    $this->ssl_gateway->send_sms($phone, $msg);
                }
            }
        } else {
            // Fallback for direct single recipient sending (e.g. reminders, alerts)
            $decoded_message = urldecode($message);
            $this->ssl_gateway->send_sms($to, $decoded_message);
        }
    }
