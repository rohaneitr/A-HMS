...
class Superadmin extends MX_Controller {
    ...
    function __construct() {
        ...
        // ------------------------------------------------------------------
        // PRIV-002 REMEDIATION: Dual-layer Authorization Guard with exit().
        //
        // The original code only checked in_group('superadmin') and called
        // redirect() without exit(). In CI3, redirect() outputs a Location
        // header via PHP's header() function and then RETURNS — it does not
        // halt execution. If output buffering (ob_start) is active, the
        // Location header may be held in the buffer and the controller body
        // continues executing with the attacker's request context.
        //
        // We now enforce two sequential checks:
        //   1. logged_in()  — verifies an active authenticated session exists.
        //                     Blocks unauthenticated requests entirely.
        //   2. in_group()   — verifies the authenticated user holds the
        //                     'superadmin' group membership specifically.
        //                     Blocks authenticated non-superadmin users (e.g.,
        //                     a hospital Admin who logs in cannot reach this).
        //
        // exit() after redirect() is MANDATORY — it is the only guarantee
        // that PHP execution stops before $this->load->model() or any
        // subsequent action method runs in an unauthorized context.
        // ------------------------------------------------------------------
